    ...
    public function rules()
    {
        return [
            ...
            'actual_name' => ['required', 'string', 'max:255', Rule::unique('units')->where(function ($query) {
                return $query->where('actual_name', $this->actual_name)->where('business_id', '=', $this->business_id)->where('deleted_at','=',null)->where('id','!=',$this->route()->parameter('unit'));
            })],
            'short_name'  => ['nullable', 'max:255', Rule::unique('units')->where(function ($query) {
                return $query->where('short_name', $this->short_name)->where('business_id', '=', $this->business_id)->where('deleted_at','=',null)->where('id','!=',$this->route()->parameter('unit'));
            })],
